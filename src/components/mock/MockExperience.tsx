...
function Icon({ name = "spark" }: { name?: string }) {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    {name === "arrow" ? <path d="M5 12h14m-6-6 6 6-6 6" /> : name === "pin" ? <><path d="M19 10c0 5-7 11-7 11S5 15 5 10a7 7 0 1 1 14 0Z"/><circle cx="12" cy="10" r="2"/></> : name === "food" ? <><path d="M5 3v6c0 3 5 3 5 0V3M7.5 3v18M19 21V3c-5 3-5 10 0 10"/></> : name === "sports" ? <><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3v18M6 5c8 3 8 11 0 14M18 5c-8 3-8 11 0 14"/></> : name === "events" ? <><path d="M9 18V5l11-2v13M9 8l11-2"/><ellipse cx="6" cy="18" rx="3" ry="2"/><ellipse cx="17" cy="16" rx="3" ry="2"/></> : name === "calendar" ? <><rect x="3" y="5" width="18" height="16" rx="3"/><path d="M7 3v4M17 3v4M3 11h18m-13 5h2m4 0h2"/></> : <path d="m12 2 2.7 7.3L22 12l-7.3 2.7L12 22l-2.7-7.3L2 12l7.3-2.7Z"/>}
  </svg>;
}
...
export default function MockExperience() {
  ...
  return <main className={`mock-app ${profile ? "dashboard" : "onboarding"}`} data-theme={theme} lang={locale}>
    <div className="world"><CityMap /></div>
    ...
    {!ready ? <div className="loading">{t("On prépare ta soirée…", "Getting your evening ready…")}</div> : !profile ? <>
      ...
      <div className="dashboard-body" data-mobile={mobile}>
        <section className="recommendations" aria-label={t("Recommandations", "Recommendations")}><div className="section-label"><span><Icon/> {t("CHOISI POUR TOI", "PICKED FOR YOU")}</span><span>03</span></div>{ordered.map(p => <article className={`pick-card ${selected === p.id ? "selected" : ""}`} key={p.id}>
          <div className="pick-meta"><span><Icon name={p.id}/> {p.category[locale]}</span><span>{p.time}</span></div><button className="pick-title" onClick={() => setSelected(p.id)} aria-pressed={selected === p.id}>{p.name}<span>↗</span></button><div className="venue">{p.venue}</div><p>{p[locale]}</p><div className="card-bottom"><span>{profile.interests.includes(p.id) ? t("Selon tes envies", "Matches your interests") : t("À découvrir", "Something to discover")}</span><button onClick={() => { setSelected(p.id); setMobile("map"); }} aria-label={`${t("Voir sur la carte", "View on map")}: ${p.name}`}><Icon name="pin"/></button></div>
        </article>)}<p className="fiction-note">{t("Suggestions fictives pour explorer la démo.", "Fictional suggestions to explore the demo.")}</p></section>
        <section className="map-stage" aria-label={t("Carte schématique de Montréal", "Schematic map of Montréal")}><div className="map-inner"><CityMap/>{picks.map((p, i) => <button key={p.id} className={`map-marker ${selected === p.id ? "active" : ""}`} style={{ left: `${p.x}%`, top: `${p.y}%` }} aria-label={`${t("Repère", "Map pin")} ${i + 1}: ${p.name}`} aria-pressed={selected === p.id} onClick={() => setSelected(p.id)}>{i + 1}<span>{p.venue}</span></button>)}</div><div className="map-north" aria-hidden="true">N<br/>↑</div><div className="map-detail" aria-live="polite"><div className="eyebrow">{pick.category[locale]} · {pick.time}</div><h2>{pick.name}</h2><p><Icon name="pin"/>{pick.address}</p><button className="primary" disabled={agenda.includes(pick.id)} onClick={() => { setAgenda(prev => prev.includes(pick.id) ? prev : [...prev, pick.id]); setNotice(t(`${pick.name} ajouté à ton agenda.`, `${pick.name} added to your agenda.`)); }}><Icon name="calendar"/>{agenda.includes(pick.id) ? t("Ajouté à l’agenda ✓", "Added to agenda ✓") : t("Ajouter au calendrier", "Add to calendar")}</button></div><div className="map-caption">{t("MONTRÉAL · CARTE ILLUSTRATIVE", "MONTRÉAL · ILLUSTRATIVE MAP")}<span>──── &nbsp; 500 m</span></div></section>
        <section className="agenda panel" aria-label="Agenda"><div className="panel-heading"><Icon name="calendar"/><h2>{t("Ton agenda", "Your agenda")}</h2></div><div className="agenda-date"><strong>12</strong><span>{t("SAMEDI", "SATURDAY")}<br/>SEPT. 2026</span></div><div className="agenda-timeline">{[15, 16, 17, 18, 19, 20, 21].map(hour => <div className="time-slot" key={hour}><time>{hour}:00</time>{hour === 15 && <div className="agenda-event muted-event"><strong>{t("Une petite balade", "A little walk")}</strong><small>Mont-Royal · 15:00</small></div>}{picks.filter(p => Number(p.time.slice(0, 2)) === hour && agenda.includes(p.id)).map(p => <div className="agenda-event" key={p.id}><strong>{p.name}</strong><small>{p.time} — {p.end}</small><button aria-label={`${t("Retirer", "Remove")} ${p.name}`} onClick={() => { setAgenda(prev => prev.filter(id => id !== p.id)); setNotice(t("Activité retirée.", "Activity removed.")); }}>×</button></div>)}</div>)}</div><div className="agenda-footer">{t("De la place pour l’imprévu.", "Leave a little room for serendipity.")}</div></section>
        <section className="chat panel" aria-label={t("Assistant de démonstration", "Demo assistant")}><div className="panel-heading"><span className="assistant-icon"><Icon/></span><div><h2>maj$q</h2><small>{t("Ton complice de sortie", "Your going-out companion")}</small></div><span className="chat-status" title={t("Démo locale", "Local demo")}/></div><div className="chat-messages" ref={conversation} role="log" aria-label={t("Conversation", "Conversation")} aria-live="polite"><div className="chat-date">{t("LE DÉBUT D’UNE BELLE SOIRÉE", "THE START OF A GOOD EVENING")}</div><div className="message assistant"><span className="message-label">maj$q</span><p>{t(`Salut ${profile.name}. J’ai trois idées pour toi, juste ici à Montréal.`, `Hey ${profile.name}. I’ve found three ideas for you, right here in Montréal.`)}</p><p>{t("On construit ta soirée ensemble ?", "Want to put an evening together?")}</p></div>{profile.note && <div className="message user"><span className="message-label">{t("TES ENVIES", "ON YOUR MIND")}</span><p>{profile.note}</p></div>}{messages.map((m, i) => <div key={i} className={`message ${m.role}`}><span className="message-label">{m.role === "user" ? profile.name : "maj$q"}</span><p>{m.text}</p></div>)}</div><div className="chat-bottom"><div className="suggestions">{[t("Un bon resto ?", "Somewhere to eat?"), t("Du jazz ce soir", "Live music tonight"), t("Un peu de sport", "A little sport")].map(s => <button key={s} onClick={() => send(s)}>{s} ↗</button>)}</div><form className="chat-input" onSubmit={e => { e.preventDefault(); send(draft); }}><label className="sr-only" htmlFor="message">{t("Ton message", "Your message")}</label><input id="message" maxLength={1000} value={draft} onChange={e => setDraft(e.target.value)} placeholder={t("Une envie ? Dis-moi…", "What are you in the mood for?")}/><button disabled={!draft.trim()} aria-label={t("Envoyer", "Send")}><Icon name="arrow"/></button></form><small className="chat-disclaimer">{t("Réponses simulées · Juste pour essayer", "Scripted replies · Made for exploring")}</small></div></section>
      </div>
    </>}
