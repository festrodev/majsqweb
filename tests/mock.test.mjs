...
for (const value of [null, '{broken', '{}', JSON.stringify({ ...profile, name: ' ' }), JSON.stringify({ ...profile, interests: [], note: ' ' }),JSON.stringify({ ...profile, interests: ['unknown'] }), JSON.stringify({ ...profile, locale: 'xx' })]) {
...
// The agenda grid spans 15:00–23:00 and stacks nothing, so events must fit and never overlap.
const slots = [...calendar, ...picks].sort((a, b) => a.time.localeCompare(b.time));
slots.forEach((e, i) => {
  assert.ok(e.time >= '15:00' && e.time < e.end && e.end <= '23:00', `${e.id} fits the agenda grid`);
